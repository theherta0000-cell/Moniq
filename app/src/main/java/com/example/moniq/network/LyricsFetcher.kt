...
object LyricsFetcher {
    ...
    fun buildUrl(title: String?, artist: String?, album: String?, durationSec: Int?, baseUrl: String = "https://lyricsplus.prjktla.workers.dev/v2/lyrics/get"): String {
    ...
    suspend fun fetchLyrics(title: String?, artist: String?, album: String?, durationSec: Int?): LyricsResult {
        return withContext(Dispatchers.IO) {
            try {
                // Try workers.dev with first referer
                val url1 = buildUrl(title, artist, album, durationSec)
                val req1 = Request.Builder().url(url1)
                ...
                if (!resp.isSuccessful) {
                    ...
                    val req2 = Request.Builder().url(url1)
                        .header("User-Agent", "Moniq/lyrics-fetcher")
                        .header("Accept", "application/json")
                ...
                if (!resp.isSuccessful) {
                    try { resp.close() } catch (_: Exception) {}
                    val url2 = buildUrl(title, artist, album, durationSec, "https://lyrics-plus-backend.vercel.app/v2/lyrics/get")
                    val req3 = Request.Builder().url(url2)
                        .header("User-Agent", "Moniq/lyrics-fetcher")
                        .header("Accept", "application/json")
    ...
    val body = resp.body?.string() ?: return@withContext LyricsResult(emptyList(), null)
    val obj = JSONObject(body)
    val arr = obj.optJSONArray("lyrics") ?: return@withContext LyricsResult(emptyList(), null)
...
if (syllArr == null || syllArr.length() == 0) {
    ...
    if (lineText.isNotEmpty()) {
        // Get transliteration text if available (not as array, but as direct text)
        val translitText = translitObj?.optString("text")
        sylls.add(com.example.moniq.lyrics.Syllable(lineTime, lineDuration, lineText, translitText))
    }
} else {
...
val successUrl = when {
    resp.request.url.host.contains("workers.dev") -> resp.request.url.toString()
    resp.request.url.host.contains("vercel.app") -> resp.request.url.toString()
