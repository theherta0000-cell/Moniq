...
/**
 * Simplified lyrics view: renders whole lines centered vertically with transliterations and
 * translations. Highlights the current line and centers it when in focused mode.
 */
class LyricsView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
        ...
        for (sl in syllableLines) {
            ...
            val translitTv =
                    TextView(context).apply {
                        ...
                        val isDuplicate =
                                !translitText.isNullOrEmpty() &&
                                        translitText
                                                .trim()
                                                .equals(lineText.trim(), ignoreCase = true)
                        visibility =
                                if ((translitText.isNullOrEmpty() ||
                                                !showTransliteration ||
                                                isDuplicate)
                                )
                                        GONE
    ...
    fun setFocusedMode(enabled: Boolean) {
        ...
        for ((i, tv) in textViews.withIndex()) {
            if (enabled) {
                tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, if (i in activeIndices) 32f else 18f)
                tv.setTextColor(if (i in activeIndices) accentColor else Color.LTGRAY)
                transliterationViews.getOrNull(i)?.apply {
                    setTextSize(TypedValue.COMPLEX_UNIT_SP, if (i in activeIndices) 15f else 13f)
                }
                translationViews.getOrNull(i)?.apply {
                    setTextSize(TypedValue.COMPLEX_UNIT_SP, if (i in activeIndices) 16f else 14f)
                }
            } else {
    ...
    fun updatePosition(posMs: Long) {
        ...
        // Find ALL lines that overlap with current position
        ...
        // Handle case where we're past all lines
        if (newActiveIndices.isEmpty() && posMs >= lines.last().endMs) {
            newActiveIndices.add(lines.size - 1)
        }

        // Find indices to remove (no longer active)
        val toRemove = activeIndices - newActiveIndices
        // Find indices to add (newly active)
        val toAdd = newActiveIndices - activeIndices

        // If nothing changed, just update syllables for active lines
        ...
        // Unhighlight lines that are no longer active
        for (idx in toRemove) {
            if (idx in textViews.indices) {
                val tv = textViews[idx]
                tv.animate().scaleX(1.0f).scaleY(1.0f).alpha(1.0f).setDuration(180).start()
                tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, if (focusedMode) 18f else 18f)
                tv.setTextColor(Color.LTGRAY)
                transliterationViews.getOrNull(idx)?.apply {
                ...
                translationViews.getOrNull(idx)?.apply {
        ...
        // Highlight newly active lines
        for (idx in toAdd) {
            if (idx in textViews.indices) {
                val tv = textViews[idx]
                tv.animate().scaleX(1.08f).scaleY(1.08f).alpha(1.0f).setDuration(200).start()
                tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, if (focusedMode) 32f else 20f)
        ...
        // Highlight syllables for all active lines
        for (idx in activeIndices) {
            if (idx in lines.indices) highlightSyllable(idx, posMs)
        }

        // Center on the first active line (or last active if multiple)
        if (activeIndices.isNotEmpty()) {
            val primaryIdx = activeIndices.maxOrNull() ?: activeIndices.first()
            if (primaryIdx in textViews.indices) {
                val tv = textViews[primaryIdx]
                post {
                    val lineContainer = tv.parent as? ViewGroup
                    if (lineContainer != null) {
                        val target =
                                (lineContainer.top - height / 2 + lineContainer.height / 2)
                                        .coerceAtLeast(0)
    ...
    private fun highlightSyllable(lineIndex: Int, posMs: Long) {
        ...
        for (i in li.syllables.indices) {
            ...
            val end =
                    if (i + 1 < li.syllableOffsets.size) li.syllableOffsets[i + 1]
                    else li.text.length
            ...
            if (i < sIdx) {
                ...
                val progress =
                        if (s.durationMs > 0) {
                            ((posMs - s.startMs).toFloat() / s.durationMs.toFloat()).coerceIn(
                                    0f,
                                    1f
                            )
                        } else 1f
    ...
    fun setAccentColor(color: Int) {
        try {
            ...
            if (activeIndices.isNotEmpty()) {
                val firstActive = activeIndices.firstOrNull() ?: 0
                updatePosition((lines.getOrNull(firstActive)?.startMs ?: 0L))
            }
        } catch (_: Exception) {}
    }
