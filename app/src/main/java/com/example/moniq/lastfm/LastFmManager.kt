 ...
 * Scrobble a track
 */
suspend fun scrobble(title: String, artist: String, album: String?, duration: Int) { // <-- Removed timestamp parameter
    if (!isAuthenticated()) return
    ...
    withContext(Dispatchers.IO) {
        try {
            ...
            if (response.isSuccessful) {
                Log.d("LastFm", "Scrobbled: $title by $cleanedArtist")
            } else {
                Log.e("LastFm", "Scrobble failed: $body")
            }
        } catch (e: Exception) {
...
suspend fun checkAndScrobble(trackId: String, title: String, artist: String, album: String?, currentPositionMs: Long, durationMs: Long) {
    // Add this very first line to confirm the function is entered
    ...
    // Add this log to see if we pass the auth check
    ...
    if (state == null) {
        ...
        // Log what the currentScrobble *actually* contains for comparison
        synchronized(this) {
    ...
    if (shouldScrobble) {
        Log.d("LastFm", "DEBUG: checkAndScrobble - Attempting to scrobble '$title' by '$artist' with duration: ${durationSec}s")
        scrobble(title, artist, album, durationSec) // Removed timestamp
        synchronized(this) {
            currentScrobble?.let {
                if (it.trackId == trackId) {
                    currentScrobble = it.copy(scrobbled = true)
                    Log.d("LastFm", "DEBUG: checkAndScrobble - Marked scrobble state for '$title' as scrobbled=true.")
                }
            }
        }
    } else {
        Log.d("LastFm", "DEBUG: checkAndScrobble - Conditions not met, not scrobbling '$title' yet.")
    }
}
